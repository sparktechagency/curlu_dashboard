...
const CreateSalonCategoryFrom = ({
  ...
  return (
    <Form form={form} layout="vertical" onFinish={onFinish} autoComplete="off">
      ...
      >
        <label
          className="flex justify-start items-center gap-2 border w-full py-3 px-2 rounded-md overflow-hidden"
          htmlFor="idCard"
        >
          {image?.idCard ? (
            <p>{image?.idCard?.name}</p>
          ) : selectedCategory?.img ? (
            <p className="">{selectedCategory?.img?.split('/').pop()}</p>
          ) : (
