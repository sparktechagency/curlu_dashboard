...
const SliderSetting = () => {
  ...
  const handleAddOrUpdateSlider = async (values) => {
    ...
    setLoading(true);
    if (selectedSlider) {
      await updateSlider({ id: selectedSlider.id, sliderData: formData })
        .unwrap()
        .then((res) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire(
            'Error',
            'Error deleting slider. Please try again.',
            'error'
          );
        ...
        <div className="p-5">
          ...
          >
            <Form.Item
              ...
              label="Slider Name"
              rules={[
                { required: true, message: 'Please input the slider name' },
              ]}
            >
              <Input />
            </Form.Item>
              ...
              <Form.Item name="image">
                <div className="flex justify-center items-center w-full h-full border-dashed border border-black py-10">
                  {imgFile ? (
                    ...
                    <img
                      src={generateImage(selectedSlider.slider_image)}
                      alt="Existing Slider"
                      style={{ width: '100px' }}
                    />
                  ) : (
